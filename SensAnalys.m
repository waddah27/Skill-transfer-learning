...
Lmt=lst+1.2*Lom;
MA=1.5; %cm
lb=[0];%, 1,-3, 0.9*MA,0,0]; %lower bounds
ub=[1];%, 10, -0.001,1.1*MA,1,1]; %Upper bounds
...
J=zeros(1,5);
J1=[1.5623,1.1360,5.8900,3.3021,1.9340,1.0657];
...
for i=2:1:3
    ...
    [solution2,objectiveValue2] = fmincon(objfun,x0,[],[],[],[],lb,ub,[],options)
    J2(i) = NMS(solution2,filtx11, force, time,Samp_freq, phi_0);
    if J2(i)<J2(i-1)
        Best_val=J2(i);
        ind=i;
    ...
    Enhancement = (J(ind)-Best_val)/J(ind)*100
end
